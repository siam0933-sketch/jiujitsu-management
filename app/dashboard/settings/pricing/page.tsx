                    ...
                    <div className="space-y-12">
                        ...
                        <section>
                            <h2 className="text-lg font-bold text-gray-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
                                ...
                                기간권 관리
                                <span className="text-xs font-normal text-gray-500 dark:text-zinc-400">(기본 1개월 단위)</span>
                            </h2>
                            ...
                            <div className="border rounded-md p-4 bg-white dark:bg-zinc-900">
                                <h3 className="font-semibold text-gray-900 dark:text-zinc-100 mb-3 text-sm border-b border-gray-100 dark:border-zinc-800/50 pb-2">등록된 기간권 목록</h3>
                                <ul className="space-y-3">
                                    {filteredPlans.map(plan => (
                                        <li key={plan.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                                            <div>
                                                <span className="font-medium text-gray-900 dark:text-zinc-100 mr-2">{plan.name}</span>
                                                <span className="text-gray-500 dark:text-zinc-400 text-xs">/ 1개월</span>
                                            </div>
                                            <div className="flex items-center gap-4">
                                                <span className="text-gray-900 dark:text-zinc-100 font-medium">{plan.price.toLocaleString()}원</span>
                                                <button onClick={() => handleDeletePlan(plan.id)} className="text-gray-400 dark:text-zinc-500 hover:text-red-600 text-xs">삭제</button>
                                            </div>
                                        </li>
                                    ))}
                                    {filteredPlans.length === 0 && <p className="text-gray-500 dark:text-zinc-400 text-sm">등록된 기간권이 없습니다.</p>}
                                </ul>
                            </div>
                        </section>
                        ...
                        {/* Options Section */}
                        <section>
                            <h2 className="text-lg font-bold text-gray-900 dark:text-zinc-100 mb-4 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <svg className="w-5 h-5 text-gray-900 dark:text-zinc-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                    </svg>
                                    옵션 관리
                                    <span className="text-xs font-normal text-gray-500 dark:text-zinc-400">(기간권에만 적용)</span>
                                </div>
                            </h2>
                            ...
                            <div className="space-y-6">
                                {groupedOptions.map((group: any, gIdx: number) => (
                                    <div key={group.name} className="border rounded-lg bg-white dark:bg-zinc-900 overflow-hidden shadow-sm">
                                        <div className="bg-gray-50 dark:bg-zinc-800/50 px-4 py-3 border-b border-gray-100 dark:border-zinc-800/50 flex justify-between items-center">
                                                ...
                                                {editingGroupName === group.name ? (
                                                    <form onSubmit={(e) => handleUpdateGroup(group.name, e)} className="flex items-center gap-2 flex-1">
                                                        <input
                                                            ...
                                                            autoFocus
                                                            className="text-sm border border-blue-300 rounded px-2 py-0.5 w-full max-w-[200px] focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none"
                                                        />
                                                        <button type="submit" className="text-xs bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-500">저장</button>
                                                        <button type="button" onClick={() => setEditingGroupName(null)} className="text-xs bg-gray-200 text-gray-700 dark:text-zinc-300 px-2 py-1 rounded hover:bg-gray-300">취소</button>
                                                    </form>
                                                ) : (
                                                    <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setEditingGroupName(group.name)}>
                                                        <h3 className="font-bold text-gray-900 dark:text-zinc-100 text-sm hover:text-blue-600 transition-colors">{group.name}</h3>
                                                        <svg className="w-3 h-3 text-gray-400 dark:text-zinc-500 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                        </svg>
                                                        <span className="text-[10px] text-gray-500 dark:text-zinc-400 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 px-1.5 rounded ml-2 cursor-default" onClick={e => e.stopPropagation()}>{group.items.length}개 옵션</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <ul className="divide-y divide-gray-50">
                                            {group.items.map((opt: any, idx: number) => (
                                                <li key={opt.id} className="flex justify-between items-center text-sm px-4 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800/50 dark:bg-zinc-800/50 dark:hover:bg-zinc-800 dark:bg-zinc-800/50 transition-colors">
                                                    {editingOptionId === opt.id ? (
                                                        <form onSubmit={(e) => handleUpdateOption(opt.id, e)} className="flex items-center gap-2 w-full">
                                                            <div className="flex items-center gap-2 flex-1">
                                                                <input name="name" defaultValue={opt.name} className="flex-1 text-sm border border-blue-300 rounded px-2 py-1 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" autoFocus />
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <input name="price" type="number" defaultValue={opt.price} className="w-24 text-sm border border-blue-300 rounded px-2 py-1 text-right bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" />
                                                                <button type="submit" className="text-xs bg-blue-600 text-white px-2 py-1 rounded">저장</button>
                                                                <button type="button" onClick={() => setEditingOptionId(null)} className="text-xs bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 px-2 py-1 rounded">취소</button>
                                                            </div>
                                                        </form>
                                                    ) : (
                                                        <>
                                                            <div className="flex items-center gap-2 flex-1">
                                                                ...
                                                                <span className="text-gray-700 dark:text-zinc-300 ml-2 cursor-pointer hover:text-blue-600 flex items-center gap-1 group" onClick={() => setEditingOptionId(opt.id)}>
                                                                    {opt.name}
                                                                    <svg className="w-3 h-3 text-gray-300 dark:text-zinc-600 opacity-0 group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                                    </svg>
                                                                </span>
                                                            </div>
                                                            <div className="flex items-center gap-4">
                                                                <span className={`font-medium cursor-pointer hover:text-blue-600 ${opt.price > 0 ? 'text-blue-600' : 'text-gray-400 dark:text-zinc-500'}`} onClick={() => setEditingOptionId(opt.id)}>
                                                                    {opt.price > 0 ? '+' : ''}{opt.price.toLocaleString()}원
                                                                </span>
                                                                <button onClick={() => handleDeleteOption(opt.id)} className="text-gray-300 dark:text-zinc-600 hover:text-red-500 p-1 rounded hover:bg-red-50 transition-all">
                                            ...
                                            <li className="bg-gray-50 dark:bg-zinc-800/50/50 px-4 py-3 border-t border-dashed border-gray-200 dark:border-zinc-800">
                                                <form onSubmit={handleCreateOption} className="flex gap-2 items-center">
                                                    <input type="hidden" name="group_name" value={group.name} />
                                                    <span className="text-xs text-gray-400 dark:text-zinc-500 w-6 text-center flex justify-center">
                                                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                                        </svg>
                                                    </span>
                                                    <input name="name" required placeholder="옵션 추가..." className="flex-1 bg-transparent border-0 border-b border-gray-300 dark:border-zinc-700 focus:border-blue-500 focus:ring-0 text-xs py-1 px-0 placeholder-gray-400 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" />
                                                    <input name="price" type="number" required placeholder="금액" className="w-20 bg-transparent border-0 border-b border-gray-300 dark:border-zinc-700 focus:border-blue-500 focus:ring-0 text-xs py-1 px-0 text-right placeholder-gray-400 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" />
                                ...
                                {groupedOptions.length === 0 && <p className="text-center text-gray-400 dark:text-zinc-500 text-sm py-8">등록된 옵션 그룹이 없습니다. 위에서 그룹을 생성해주세요.</p>}
                            </div>
                        </section>
                    </div>
                )}
                    ...
                    <div>
                        ...
                        <div className="mb-8 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-lg border border-gray-200 dark:border-zinc-800">
                            ...
                            <form onSubmit={handleCreatePlan} className="flex gap-2 items-end">
                                ...
                                <div className="flex-1">
                                    <label className="block text-xs font-medium text-gray-700 dark:text-zinc-300 mb-1">이용권 이름</label>
                                    <input name="name" required placeholder="예: 10회 이용권" className="block w-full rounded-md border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-zinc-100 shadow-sm ring-1 ring-inset ring-transparent placeholder:text-gray-400 dark:text-zinc-500 dark:placeholder:text-zinc-500 focus:ring-2 focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 appearance-none" />
                                </div>
                                <div className="w-24">
                                    <label className="block text-xs font-medium text-gray-700 dark:text-zinc-300 mb-1">기본 금액</label>
                                    <input name="price" type="number" required placeholder="0" className="block w-full rounded-md border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-zinc-100 shadow-sm ring-1 ring-inset ring-transparent placeholder:text-gray-400 dark:text-zinc-500 dark:placeholder:text-zinc-500 focus:ring-2 focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 appearance-none" />
                                </div>
                                <div className="w-20">
                        ...
                        <div className="border rounded-md p-4 bg-white dark:bg-zinc-900">
                            <h3 className="font-semibold text-gray-900 dark:text-zinc-100 mb-3 text-sm border-b border-gray-100 dark:border-zinc-800/50 pb-2">등록된 횟수권 목록</h3>
                            <ul className="space-y-3">
                                {filteredPlans.map(plan => (
                                    <li key={plan.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0">
                                        <div>
                                            <span className="font-medium text-gray-900 dark:text-zinc-100 mr-2">{plan.name}</span>
                                            <span className="text-gray-500 dark:text-zinc-400 text-xs">({plan.session_count}회 / {plan.duration_days}일 유효)</span>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className="text-gray-900 dark:text-zinc-100 font-medium">{plan.price.toLocaleString()}원</span>
                                            <button onClick={() => handleDeletePlan(plan.id)} className="text-gray-400 dark:text-zinc-500 hover:text-red-600 text-xs">삭제</button>
                                ...
                                {filteredPlans.length === 0 && <p className="text-gray-500 dark:text-zinc-400 text-sm">등록된 횟수권이 없습니다.</p>}
                            </ul>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'product' && (
                    <div>
                        <h2 className="text-lg font-bold text-gray-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
                            ...
                            상품 관리
                            <span className="text-xs font-normal text-gray-500 dark:text-zinc-400">(도복, 벨트 등 물품)</span>
                        </h2>
                        ...
                        <div className="border rounded-md p-4 bg-white dark:bg-zinc-900">
                            <h3 className="font-semibold text-gray-900 dark:text-zinc-100 mb-3 text-sm border-b border-gray-100 dark:border-zinc-800/50 pb-2">등록된 상품 목록</h3>
                            <ul className="space-y-3">
                                {products.map((product, idx) => (
                                    <li key={product.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2 last:border-0 last:pb-0 hover:bg-gray-50 dark:hover:bg-zinc-800/50 dark:bg-zinc-800/50 dark:hover:bg-zinc-800 dark:bg-zinc-800/50 transition-colors p-1">
                                        {editingProductId === product.id ? (
                                            <form onSubmit={(e) => handleUpdateProduct(product.id, e)} className="flex items-center gap-2 w-full">
                                                <div className="flex items-center gap-2 flex-1">
                                                    <input name="name" defaultValue={product.name} className="flex-1 text-sm border border-blue-300 rounded px-2 py-1 bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" autoFocus />
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <input name="price" type="number" defaultValue={product.price} className="w-24 text-sm border border-blue-300 rounded px-2 py-1 text-right bg-white dark:bg-zinc-900 text-gray-900 dark:text-zinc-100 dark:border-zinc-700 appearance-none" />
                                                    <button type="submit" className="text-xs bg-blue-600 text-white px-2 py-1 rounded">저장</button>
                                                    <button type="button" onClick={() => setEditingProductId(null)} className="text-xs bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 px-2 py-1 rounded">취소</button>
                                                </div>
                                            </form>
                                        ) : (
                                            <>
                                                <div className="flex items-center gap-2 flex-1">
                                                    ...
                                                    <span className="font-medium text-gray-900 dark:text-zinc-100 mr-2 cursor-pointer hover:text-blue-600 flex items-center gap-2 group" onClick={() => setEditingProductId(product.id)}>
                                                        {product.name}
                                                        <svg className="w-3 h-3 text-gray-300 dark:text-zinc-600 opacity-0 group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                        </svg>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <span className="text-gray-900 dark:text-zinc-100 font-medium cursor-pointer hover:text-blue-600" onClick={() => setEditingProductId(product.id)}>{product.price.toLocaleString()}원</span>

                                                    <button onClick={() => handleCopyProduct(product.id)} className="text-gray-400 dark:text-zinc-500 hover:text-blue-600 text-xs p-1 rounded hover:bg-blue-50" title="복사">
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                                        </svg>
                                                    </button>
                                                    <button onClick={() => handleDeleteProduct(product.id)} className="text-gray-400 dark:text-zinc-500 hover:text-red-600 text-xs p-1 rounded hover:bg-red-50" title="삭제">
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                        </svg>
                                                    </button>
                                ...
                                {products.length === 0 && <p className="text-gray-500 dark:text-zinc-400 text-center text-sm py-8">등록된 상품이 없습니다.</p>}
                            </ul>
                        </div>
                    </div>
                )}
